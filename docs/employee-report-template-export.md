...
### Svrha
`EmployeeReportTemplateExport` generira mjesečni ili godišnji Excel izvještaj radnih sati zaposlenika popunjavanjem podataka u postojeći XLSX predložak ("evidencija radnog vremena"). Predložak sadrži 12 sheetova (jedan za svaki mjesec) s predefiniranim rasporedom ćelija, formulama i formatiranjem.

Podržava dva moda rada:
- **Pojedinačni mjesec** (`download()`) — učitava samo potreban sheet za performanse
- **Svi mjeseci** (`downloadAllMonths()`) — učitava cijeli template i popunjava svih 12 sheetova u jednom XLSX-u
...
│  │  2. Load XLSX (1 sheet)   │  ← setLoadSheetsOnly()          │
│  │  3. fillSheet()           │  ← centralna metoda              │
│  │  4. saveToTemp()          │                                  │
│  │  5. Return download       │                                  │
│  │                           │                                  │
│  │ downloadAllMonths() — 12: │                                  │
│  │  1. getTemplatePath()     │                                  │
│  │  2. Load XLSX (all sheets)│                                  │
│  │  3. fillSheet() × 12      │  ← za svaki mjesec              │
│  │  4. saveToTemp()          │                                  │
│  │  5. Return download       │                                  │
│  │                           │                                  │
│  │ fillSheet() internals:    │                                  │
│  │  a. Resolve row mapping   │  ← per-month (Nov/Dec shift)    │
│  │  b. Get report data ──────┼──► Employee::getMonthlyWorkReport│
│  │  c. Fill header cells     │                                  │
│  │  d. Clear old values      │                                  │
│  │  e. Fill daily data       │                                  │
│  │  f. Fill work times       │                                  │
│  │  g. Set number formats    │  ← per-row (ne rectangular)     │
│  │  h. Add SUM formulas      │                                  │
│  │  i. Apply day formatting  │  ← range-based per column       │
│  └───────────────────────────┘                                  │
└─────────────────────────────────────────────────────────────────┘
                          │
                          ▼
...
Za svaki radni dan (ne vikend, ne praznik), ili vikend/praznik ako zaposlenik ima unesene sate:
- `{col}7` = početak rada (default iz `TenantFeatureService::getDefaultStartTime()`, ili stvarni `work_start_time` iz TimeLog-a)
- `{col}8` = završetak rada (default iz `TenantFeatureService::getDefaultEndTime()`, ili stvarni `work_end_time` iz TimeLog-a)
...
### Centralna metoda: `fillSheet(Worksheet $sheet, int $month, int $year)`

Sva logika popunjavanja je ekstrahirana u `fillSheet()` metodu koja se poziva za svaki sheet.
Row mapping (`$hourTypeRows`) se resolvira **lokalno unutar metode** na temelju mjeseca — nema class-level property-ja.
...
Number format se primjenjuje **per-row** (samo na podatkovne retke), a ne na cijeli pravokutni raspon — time se čuva template formatting na ne-podatkovnim redovima:
...
`FORMAT_GENERAL` automatski prikazuje cijele brojeve bez decimala, a decimale s do 2 decimalna mjesta.
...
Potpis metode: `applyDayFormatting(Worksheet $sheet, Carbon $monthDate, array $report)`

Formatiranje se primjenjuje **range-based per column** (1 poziv po stupcu, ne per-cell). Za svaki dan u mjesecu, formatiraju se **svi redovi od 7 do 33** (ili 7 do 34 za studeni/prosinac):
...
Mjeseci 11 i 12 imaju pomak +1 za redove nakon row 20 zbog "Mirovanje radnog odnosa". Row mapping se resolvira **lokalno u `fillSheet()`** za svaki sheet — `$defaultHourTypeRows` za mjesece 1-10, `$hourTypeRowsNovemberDecember` za 11-12. Nema class-level `$hourTypeRows` property-ja.
...
### Memory i timeout
- `download()` — `time_limit` 300s, `memory_limit` 512M (jedan sheet)
- `downloadAllMonths()` — `time_limit` 600s, `memory_limit` 1G (svih 12 sheetova)

### Temp file cleanup
Download response koristi `->deleteFileAfterSend(true)` tako da se temp file automatski briše nakon slanja. `saveToTemp()` metoda također poziva `$spreadsheet->disconnectWorksheets()` za čišćenje memorije.

### Performanse optimizacije
- **Single-month**: `setLoadSheetsOnly()` — učitava samo potreban sheet iz template-a
